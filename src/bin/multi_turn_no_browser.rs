...
fn build_inner_req_list_no_browser(
    ...
    slot3_override: Option<Value>,
    slot4_override: Option<Value>,
) -> Vec<Value> {
    ...
    // Variant handling for slots 3/4.
    match slot3_override {
        Some(v) => inner[3] = v,
        None => inner[3] = Value::Null,
    }
    match slot4_override {
        Some(v) => inner[4] = v,
        None => inner[4] = Value::Null,
    }

    // Live browser captures also populate slot 5 with an attestation UUID
    // when slots 3/4 are present.  Keep it null for the no-browser variants.
    inner[5] = Value::Null;
...
fn response_text_snippet(body: &str) -> String {
    // Try to extract first readable text from the StreamGenerate response.
...
async fn main() {
    ...
    // ---------- TURN 1: no state, no attestation ----------
    let prompt1 = "My name is Alice. Remember it.";
    let reqid1 = {
        let ts = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
    ...
    let inner1 = build_inner_req_list_no_browser(prompt1, category_enum, None, None, None);
    let body1 = build_stream_generate_body(&inner1);
    std::fs::write(format!("{out_dir}/turn1_request_body.txt"), &body1).unwrap();
    std::fs::write(
        format!("{out_dir}/turn1_inner_req_list.json"),
        serde_json::to_string_pretty(&inner1).unwrap(),
    )
    .unwrap();

    println!("\n=== TURN 1: sending without state/attestation ===");
    let resp1 = match send_turn(&client, &headers, &params1, &inner1).await {
        Ok(body) => {
            std::fs::write(format!("{out_dir}/turn1_response_raw.txt"), &body).unwrap();
            println!("TURN 1 HTTP OK; body length {}", body.len());
            let snippet = response_text_snippet(&body);
            println!("TURN 1 text snippet: {}", snippet);
            match extract_conversation_state(&body) {
                Ok(state) => {
                    println!("TURN 1 extracted state: {:?}", state);
                    std::fs::write(
                        format!("{out_dir}/turn1_state.json"),
                ...
                Err(e) => {
                    println!("TURN 1 failed to extract state: {e}");
                    None
                }
            }
        }
        Err(e) => {
            std::fs::write(format!("{out_dir}/turn1_error.txt"), &e).unwrap();
            println!("TURN 1 FAILED: {e}");
            None
        }
    };

    // If we failed to get state, stop.
    let state = match resp1 {
        Some(s) => s,
        None => {
            println!("\nNo state extracted; cannot continue.");
            return;
        }
    };
    ...
    let variants: Vec<(&str, Option<Value>, Option<Value>)> = vec![
        ("null_null", None, None),
        ("empty_empty", Some(json!("")), Some(json!(""))),
    ];

    for (name, slot3, slot4) in variants {
        let reqid2 = {
            let ts = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
        ...
        let inner2 = build_inner_req_list_no_browser(prompt2, category_enum, Some(&state), slot3, slot4);
        let body2 = build_stream_generate_body(&inner2);
        std::fs::write(format!("{out_dir}/turn2_{name}_request_body.txt"), &body2).unwrap();
        std::fs::write(
            format!("{out_dir}/turn2_{name}_inner_req_list.json"),
        ...
        match send_turn(&client, &headers, &params2, &inner2).await {
            Ok(body) => {
                std::fs::write(format!("{out_dir}/turn2_{name}_response_raw.txt"), &body).unwrap();
                println!("TURN 2 {} HTTP OK; body length {}", name, body.len());
                let snippet = response_text_snippet(&body);
                println!("TURN 2 {} text snippet: {}", name, snippet);
            }
            Err(e) => {
                std::fs::write(format!("{out_dir}/turn2_{name}_error.txt"), &e).unwrap();
                println!("TURN 2 {} FAILED: {}", name, e);
            }
        }
    }
}
